...
def _tq_hnsw_params(dim=2, metric='COSINE', compression='TQ8'):
...
def test_sanity_cosine():
    ...
    # TQ compression is outside VECSIM_ALGOS for now, so validate it explicitly on the same
    # cosine path via HNSW + COMPRESSION TQ8.
    params = _tq_hnsw_params(metric='COSINE')
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'HNSW', str(len(params)), *params).ok()
    ...
    actual_res = env.expect('FT.SEARCH', 'idx', '*=>[KNN 4 @v $blob AS dist]', 'PARAMS', '2',
                            'blob', query_vec.tobytes(), 'SORTBY', 'dist', 'RETURN', '1', 'dist').res
    env.assertGreaterEqual(actual_res[0], 3, message='TQ8/COSINE')
    env.assertEqual(actual_res[1], 'a', message='TQ8/COSINE')
    env.assertTrue('b' in actual_res, message='TQ8/COSINE')

    range_dist = spatial.distance.cosine(np.array([0.1, 0.4]), query_vec) + EPSILONS['FLOAT32']
    actual_res = env.expect('FT.SEARCH', 'idx', f'@v:[VECTOR_RANGE {range_dist} $blob]=>{{$yield_distance_as: dist}}',
                            'PARAMS', '2', 'blob', query_vec.tobytes(), 'SORTBY', 'dist', 'RETURN', '1', 'dist').res
    env.assertGreaterEqual(actual_res[0], 1, message='TQ8/COSINE')
    env.assertEqual(actual_res[1], 'a', message='TQ8/COSINE')
    ...
    actual_res = env.expect('FT.SEARCH', 'idx', '*=>[KNN 4 @v $blob AS dist]', 'PARAMS', '2',
                            'blob', query_vec.tobytes(), 'SORTBY', 'dist', 'RETURN', '1', 'dist').res
    env.assertGreaterEqual(actual_res[0], 3, message='TQ8/COSINE update')
    env.assertEqual(actual_res[1], 'a', message='TQ8/COSINE update')
    env.assertTrue('b' in actual_res, message='TQ8/COSINE update')

    conn.execute_command('DEL', 'b')
    actual_res = env.expect('FT.SEARCH', 'idx', '*=>[KNN 4 @v $blob AS dist]', 'PARAMS', '2',
                            'blob', query_vec.tobytes(), 'SORTBY', 'dist', 'RETURN', '1', 'dist').res
    env.assertGreaterEqual(actual_res[0], 3, message='TQ8/COSINE delete')
    env.assertEqual(actual_res[1], 'a', message='TQ8/COSINE delete')
    env.assertFalse('b' in actual_res, message='TQ8/COSINE delete')
...
def test_sanity_l2():
    ...
    # TQ compression does not support L2 yet; verify it is rejected with a clear error.
    params = _tq_hnsw_params(metric='L2')
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'HNSW', str(len(params)), *params) \
...
def test_sanity_zero_results():
    ...
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'HNSW', '8', 'TYPE', 'FLOAT32',
               'DIM', dim, 'DISTANCE_METRIC', 'COSINE', 'COMPRESSION', 'TQ8', 'n', 'NUMERIC').ok()
...
def test_create_errors():
    ...
    ## TQ compression (HNSW COMPRESSION argument)
    # Legacy standalone algorithm names are no longer accepted.
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'TQ-FLAT', '6', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP').error().contains('Bad arguments for vector similarity algorithm')
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'TQ-HNSW', '6', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP').error().contains('Bad arguments for vector similarity algorithm')
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'HNSW', '8', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'COMPRESSION', 'TQ16') \
        .error().contains('Bad arguments for vector similarity HNSW index `COMPRESSION`')
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'HNSW', '8', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'COMPRESSION', 'BITS') \
        .error().contains('Bad arguments for vector similarity HNSW index `COMPRESSION`')
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'HNSW', '8', 'TYPE', 'FLOAT32', 'DIM', '1024', 'DISTANCE_METRIC', 'L2', 'COMPRESSION', 'TQ8') \
        .error().contains('TQ compression with DISTANCE_METRIC L2 is not yet supported; use COSINE or IP')
    env.expect('FT.CREATE', 'idx', 'SCHEMA', 'v', 'VECTOR', 'HNSW', '8', 'TYPE', 'FLOAT64', 'DIM', '1024', 'DISTANCE_METRIC', 'IP', 'COMPRESSION', 'TQ8') \
...
def test_create_multi_value_json():
    ...
    conn.flushall()
    env.expect('FT.CREATE', 'idx', 'ON', 'JSON', 'SCHEMA', '$.path.to.vec', 'AS', 'vec', 'VECTOR', 'HNSW',
               str(len(_tq_hnsw_params(dim=dim))), *_tq_hnsw_params(dim=dim)).ok()
    env.assertEqual(to_dict(env.cmd(debug_cmd(), "VECSIM_INFO", "idx", "vec"))['IS_MULTI_VALUE'], 0, message='TQ8, single-value JSON path')

    conn.flushall()
    env.expect('FT.CREATE', 'idx', 'ON', 'JSON', 'SCHEMA', '$.vecs[*]', 'AS', 'vec', 'VECTOR', 'HNSW',
               str(len(_tq_hnsw_params(dim=dim))), *_tq_hnsw_params(dim=dim)).error().contains(
